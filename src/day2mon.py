...
def setup_logger(domain_name):
    logging.basicConfig(filename=f"logs/{domain_name}_run_day2mon.log", encoding='utf-8', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
...
if __name__ == '__main__':
    ...
    if args.scheduler_file is not None:
        ...
        if args.processes is not None:
            ...
            logging.error('Run day2mon: If node is provided, you must also set number of processes')
    else:
        logging.error('Run day2mon: Must either provide a scheduler file or node and number of processes.')
    ...
    for year in range(syr, eyr + 1):
        for month in range(smnth, emnth + 1):
            # get filenames
            raw_dict, bcsd_dict, ref_hist_dict, mdl_hist_dict = helper_modules.set_filenames(year, month, domain_config, variable_config, False)
            ...
            cmd = ('cdo', '-O', '-f', 'nc4c', '-z', 'zip_6', 'monmean', str(flnm_in), str(flnm_out))