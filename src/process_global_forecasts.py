...
def setup_logger():
    logging.basicConfig(filename=f"logs/global_processing.log", encoding='utf-8', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
    ...
    if args.mode == 1:
        ...
        if global_config['raw_forecasts']['merged_variables'] == False:
            ...
            for var in global_config['variables']:
                ...
                results.append(global_processing_modules.gauss_to_regular(global_config, year, month, var))
            
            dask.compute(results)
            logging.info(f"Gauss to regular: Coordination transformation for {year} {month} successful")
