...
def get_clas():
    ...
    parser.add_argument("-p", "--period", action="store", type=str, help="Period for which the BCSD should be executed",
                        required=False)
    parser.add_argument("-c", "--crossval", action="store", type=str,
                        help="If TRUE, do not use actual forecast for computing forecast climatology ", required=False)
    parser.add_argument("-s", "--forecast_structure", action="store", type=str,
                        help="Structure of the line-chunked forecasts (can be 5D or 4D)", required=True)
...
def setup_logger(domain_name):
    logging.basicConfig(filename=f"logs/{domain_name}_run_bcsd.log", encoding='utf-8', level=logging.INFO,
...
if __name__ == '__main__':
    ...
    for year in range(syr, eyr + 1):
        ...
        for month in range(smnth, emnth + 1):
            ...
            raw_dict, bcsd_dict, ref_hist_dict, mdl_hist_dict = helper_modules.set_filenames(domain_config,
                                                                                             variable_config, dir_dict,
                                                                                             syr_calib, eyr_calib, year,
                                                                                             month_str, False)
            # else:
            ...
            attribute_config = helper_modules.update_global_attributes(attribute_config, domain_config['bc_params'],
                                                                       coords, args.domain)
            ...
            for variable in variable_config:
                ...
                ds = helper_modules.create_4d_netcdf(bcsd_dict, attribute_config, domain_config, variable_config,
                                                     coords, variable)
                ...
                ds_obs = xr.open_mfdataset(ref_hist_dict[variable],
                                           chunks={'time': len(ds_obs.time), 'lat': 50, 'lon': 50}, parallel=True,
                                           engine='netcdf4')
                ...
                if args.forecast_structure == '5D':
                    ...
                    ds_mdl = xr.open_mfdataset(mdl_hist_dict[variable],
                                               chunks={'time': len(ds_mdl.time), 'ens': len(ds_mdl.ens), 'lat': 5,
                                                       'lon': 5}, parallel=True, engine='netcdf4')
                ...
                ds_pred = xr.open_mfdataset(raw_dict[variable],
                                            chunks={'time': len(ds_pred.time), 'ens': len(ds_pred.ens), 'lat': 50,
                                                    'lon': 50}, parallel=True, engine='netcdf4')
                ...
                for timestep in range(0, len(ds_pred.time)):
                    ...
                    with dask.config.set(**{
                        'array.slicing.split_large_chunks': False}):  # --> I really don't know why we need to silence the warning here...
                    ...
                    da_temp[timestep, :, :] = xr.apply_ufunc(
                        ...
                        da_obs_sub,
                        da_mdl_sub,
                        kwargs={'bc_params': domain_config['bc_params'],
