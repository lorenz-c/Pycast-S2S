...
def reformatarray(inpt):
    ...
    flenme_out    = fulloutdir + 'ECMWF_SEAS5_' + format(ens, '02') + '_' + format(year, '04') + format(month, '02') + '.nc'  
    ...
    ds_t2m.t2m.attrs = {'long_name': '2m_temperature', 'units': 'K', 'standard_name': 'air_temperature'}
    ds_t2min.t2min.attrs = {'long_name': 'minimum_daily_temperature_at_2m', 'units': 'K', 'standard_name': 'air_temperature'}
    ds_t2max.t2max.attrs = {'long_name': 'maximum_daily_temperature_at_2m', 'units': 'K', 'standard_name': 'air_temperature'}
    ds_ssrd.ssrd.attrs = {'long_name': 'surface_solar_radiation', 'units': 'W m-2'}
        
    ds_tp.to_netcdf(path=flenme_out, mode='w', format='NETCDF4_CLASSIC', encoding={'tp': {'dtype': 'int32', 'scale_factor': 0.001, 'zlib': True, 'complevel': 6, '_FillValue': -9999, 'chunksizes': [1, 640, 1280]}})
    ds_t2m.t2m.to_netcdf(path=flenme_out, mode='a', format='NETCDF4_CLASSIC', encoding={'t2m': {'dtype': 'int32', 'scale_factor': 0.0001, 'add_offset': 273.15, 'zlib': True, 'complevel': 6, '_FillValue': -9999, 'chunksizes': [1, 640, 1280]}})
    ds_t2min.t2min.to_netcdf(path=flenme_out, mode='a', format='NETCDF4_CLASSIC', encoding={'t2min': {'dtype': 'int32', 'scale_factor': 0.0001, 'add_offset': 273.15, 'zlib': True, 'complevel': 6, '_FillValue': -9999, 'chunksizes': [1, 640, 1280]}})
    ds_t2max.t2max.to_netcdf(path=flenme_out, mode='a', format='NETCDF4_CLASSIC', encoding={'t2max': {'dtype': 'int32', 'scale_factor': 0.0001, 'add_offset': 273.15, 'zlib': True, 'complevel': 6, '_FillValue': -9999, 'chunksizes': [1, 640, 1280]}})
    ds_ssrd.ssrd.to_netcdf(path=flenme_out, mode='a', format='NETCDF4_CLASSIC', encoding={'ssrd': {'dtype': 'int32', 'scale_factor': 0.001, 'zlib': True, 'complevel': 6, '_FillValue': -9999, 'chunksizes': [1, 640, 1280]}})
    
    
    
def global_processing(parameter):
    
    # Import the parameter file
    with open('../bcsd_parameter.json') as json_file:
    ...
    tp_flenme = glbraw + 'ECMWF-seas5_03deg_daily_' + 'tp' + '_' + format(year, '4') + format(month, '02') + '.nc'
    tp_hndle  = xr.open_dataset(tp_flenme, lock=False)

    t2m_flenme = glbraw + 'ECMWF-seas5_03deg_daily_' + 't2m' + '_' + format(year, '4') + format(month, '02') + '.nc'
    t2m_hndle  = xr.open_dataset(t2m_flenme, lock=False)

    t2min_flenme = glbraw + 'ECMWF-seas5_03deg_daily_' + 't2min' + '_' + format(year, '4') + format(month, '02') + '.nc'
    t2min_hndle  = xr.open_dataset(t2min_flenme, lock=False)

    t2max_flenme = glbraw + 'ECMWF-seas5_03deg_daily_' + 't2max' + '_' + format(year, '4') + format(month, '02') + '.nc'
    t2max_hndle  = xr.open_dataset(t2max_flenme, lock=False)

    ssrd_flenme = glbraw + 'ECMWF-seas5_03deg_daily_' + 'ssrd' + '_' + format(year, '4') + format(month, '02') + '.nc'
    ...
    for ens in range (0, 51):
        ...
        inpt_lst.append([glblout, ens, year, month, glbl_attrs, tp_hndle, t2m_hndle, t2min_hndle, t2max_hndle, ssrd_hndle])
