...
def get_clas():
    ...
    parser.add_argument("-p", "--period", action="store", type=str, help="Period for which the BCSD-evaluation should be executed", required=False)
...
def setup_logger(domain_name):
    logging.basicConfig(filename=f"logs/{domain_name}_run_eval.log", encoding='utf-8', level=logging.INFO,
...
if __name__ == '__main__':
    ...
    if args.scheduler_file is not None:
        ...
        if args.processes is not None:
            ...
            logging.error('Run BCSD-evaluation: If node is provided, you must also set number of processes')
    else:
        logging.error('Run BCSD-evaluation: Must either provide a scheduler file or node and number of processes.')
    ...
    for year in range(syr, eyr + 1):
        ...
        for month in range(smnth, emnth + 1):
            ...
            for variable in variable_config:
                ...
                ens_iqr = dta_in.quantile(0.75, dim = "ensemble", method = "midpoint") - dta_in.quantile(0.25, dim = "ensemble", method = "midpoint")
                # ENS spread
                ...
                # extremes
                extreme_ref  = xr.open_mfdataset(extreme_ref_fle, parallel = True, engine='netcdf4')
                ...
                extreme_ref = extreme_ref.expand_dims(dim={"ensemble": dta_in.ensemble.values})
                extreme_ref = extreme_ref[variable]

                # qunitiles
                quintile_ref = xr.open_mfdataset(quintile_ref_fle, parallel = True, engine='netcdf4')
                # Create new Dimension ensemble, in order to calculate the categories
                quintile_ref = quintile_ref.expand_dims(dim={"ensemble": dta_in.ensemble.values})
                quintile_ref = quintile_ref[variable]

                # tercile
                # qunitiles
                tercile_ref = xr.open_mfdataset(tercile_ref_fle, parallel = True, engine='netcdf4')
                # Create new Dimension ensemble, in order to calculate the categories
                tercile_ref  = tercile_ref .expand_dims(dim={"ensemble": dta_in.ensemble.values})
                ...
                # extremes
                extreme_ref = extreme_ref.rename({"month": "time"}).assign_coords(time=dta_in.time.values)


                # quintiles
                quintile_ref = quintile_ref.rename({"month": "time"}).assign_coords(time=dta_in.time.values)


                # tercile
                tercile_ref = tercile_ref.rename({"month": "time"}).assign_coords(time=dta_in.time.values)
                ...
                extreme_name = [1, 2]
                extreme_0 = xr.where(dta_in < extreme_ref[:,:,0,:,:], 1, 0).assign_coords({"quantiles": extreme_name[0]}).expand_dims("quantiles") # expand dims "quantiles" for later merging
                extreme_1 = xr.where(dta_in > extreme_ref[:,:,1,:,:], 1, 0).assign_coords({"quantiles": extreme_name[1]}).expand_dims("quantiles")
                # Write in one xarray
                ...
                quintile_0 = xr.where(dta_in < quintile_ref[:,:,0,:,:], 1, 0).assign_coords({"quantiles": quintile_name[0]}).expand_dims("quantiles") # assign and expand dimension for quintile (in order to desribe the range 0-0.2, we take the mean)
                quintile_1 = xr.where((dta_in >= quintile_ref[:,:,0,:,:]) & (dta_in < quintile_ref[:,:,1,:,:]), 1, 0).assign_coords({"quantiles": quintile_name[1]}).expand_dims("quantiles")
                quintile_2 = xr.where((dta_in >= quintile_ref[:,:,1,:,:]) & (dta_in < quintile_ref[:,:,2,:,:]), 1, 0).assign_coords({"quantiles": quintile_name[2]}).expand_dims("quantiles")
                quintile_3 = xr.where((dta_in >= quintile_ref[:,:,2,:,:]) & (dta_in < quintile_ref[:,:,3,:,:]), 1, 0).assign_coords({"quantiles": quintile_name[3]}).expand_dims("quantiles")
                quintile_4 = xr.where(dta_in >= quintile_ref[:,:,3,:,:], 1, 0).assign_coords({"quantiles": quintile_name[4]}).expand_dims("quantiles")

                # Write in one xarray
                quintile = xr.merge([quintile_0,  quintile_1,  quintile_2,  quintile_3,  quintile_4])
                ...
                tercile_name = [1, 2, 3]
                tercile_0 = xr.where(dta_in < tercile_ref[:,:,0,:,:], 1, 0).assign_coords({"quantiles": tercile_name[0]}).expand_dims("quantiles") # assign and expand dimension for tercile (in order to desribe the range 0-0.33, we take the mean)
                tercile_1 = xr.where((dta_in >= tercile_ref[:,:,0,:,:]) & (dta_in < tercile_ref[:,:,1,:,:]), 1, 0).assign_coords({"quantiles": tercile_name[1]}).expand_dims("quantiles")
                tercile_2 = xr.where(dta_in < tercile_ref[:,:,1,:,:], 1, 0).assign_coords({"quantiles": tercile_name[2]}).expand_dims("quantiles")
                ...
                mask = xr.where(dta_in[0,0,:,:] * 0 == 0, 1, np.nan) # all real values multiplied with 0 will lead to 0 and will be replaced by 1, otherwise nan multiplied with zero results in nan and will replaced by nan
                ...
                if variable == 'tp':
                    ...
                    # start here
                    max_categ_quintile = max_categ_quintile.where((extreme_ref[0, :, 1, :, :] >= 1) | (np.isnan(extreme_ref[0, :, 1, :, :])), -0.5) # we have to select the "first" ensemble member of reference, because we artificially created that dimension for processing, and we have to keep our nans
                    max_categ_tercile = max_categ_tercile.where((extreme_ref[0, :, 1, :, :] >= 1) | (np.isnan(extreme_ref[0, :, 1, :, :])), -0.5)
                    max_categ_extreme = max_categ_extreme.where((extreme_ref[0, :, 1, :, :] >= 1) | (np.isnan(extreme_ref[0, :, 1, :, :])), -0.5)
                ...
                coords = {'time': dta_in['time'].values, 'lat': dta_in['lat'].values.astype(np.float32), 'lon': dta_in['lon'].values.astype(np.float32)}
                ...
                ds = modules.create_3d_netcdf(f"{dir_dict['monthly_eval']}/{fnme_out}", attribute_config, variable, variable_config, coords)
                ...
                for variable_eval in variable_eval_config:
                    ...
                    if variable_eval == "ensemble_mean":
                        ds_out_sel[variable_eval].values = ens_mean.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "ensemble_median":
                        ds_out_sel[variable_eval].values = ens_median.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "ensemble_spread":
                        ds_out_sel[variable_eval].values = ens_spread.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "ensemble_std":
                        ds_out_sel[variable_eval].values = ens_std.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "ensemble_iqr":
                        ds_out_sel[variable_eval].values = ens_iqr.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "ensemble_anomaly":
                        ds_out_sel[variable_eval].values = delta_clim.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "ensemble_mean_relative":
                        ds_out_sel[variable_eval].values = ens_mean_rel.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "tercile_probab":
                        ds_out_sel[variable_eval].values = max_categ_tercile.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "quintile_probab":
                        ds_out_sel[variable_eval].values = max_categ_quintile.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "extreme_probab":
                        ds_out_sel[variable_eval].values = max_categ_extreme.transpose('time', 'lat', 'lon').values
                    elif variable_eval == "above_normal_probab":
                        ds_out_sel[variable_eval].values = tercile_sum[2,:,:,:].transpose('time', 'lat', 'lon').values
                    elif variable_eval == "below_normal_probab":
                        ds_out_sel[variable_eval].values = tercile_sum[0,:,:,:].transpose('time', 'lat', 'lon').values
                    elif variable_eval == "extreme_high_probab":
                        ds_out_sel[variable_eval].values = extreme_sum[1,:,:,:].transpose('time', 'lat', 'lon').values
                    elif variable_eval == "extreme_low_probab":
                        ds_out_sel[variable_eval].values = extreme_sum[0,:,:,:].transpose('time', 'lat', 'lon').values

                    ds_out_sel.to_netcdf(f"{dir_dict['monthly_eval']}/{fnme_out}", mode='a', format='NETCDF4_CLASSIC', engine='netcdf4', encoding = {variable_eval: encoding[variable_eval]})
