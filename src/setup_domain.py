...
if data_set  == "seas5":
    ...
    for year in range(syr_calib,eyr_calib+1):
        ...
        for month in range (smonth_calib,emonth_claib+1):
            ...
            ds = xr.open_mfdataset(seas5_raw_dir + year_str + '/' + month_str + '/*.nc', concat_dim = "ens", combine = "nested", parallel = True)
            ...
            seas5_raw_flne = reg_dir + 'SEAS5_daily_' + year_str + month_str + '_O320_' + domain + '.nc'
            seas5_high_flne = seas5_high_dir + 'SEAS5_daily_' + year_str + month_str + '_0.1_' + domain + '.nc'
    ...
    for var in vars:
        ...
        for year in range(syr_calib,eyr_calib+1):
            year_str = str(year)
            # Open dataset
            ds = xr.open_mfdataset(era5_raw_dir + "ERA5_Land_daily_" + var + "_" + year_str + ".nc")
