...
@dask.delayed
def bc_module(da_pred, da_obs, da_mdl, lat_index, lon_index, da_out, domain_config, precip):
    ...
    da_pred_pixel = da_pred.isel(lat=lat_index, lon=lon_index).persist()
    da_obs_pixel = da_obs.isel(lat=lat_index, lon=lon_index).persist()
    da_mdl_pixel = da_mdl.isel(lat=lat_index, lon=lon_index).persist()
    ...
    out = da_pred_pixel.copy()
    out[:] = np.nan
    
    for timestep in range(0, len(da_pred_pixel.time)):
        
        print(f"Timestep {timestep}")
        
        intersection_day_obs, intersection_day_mdl = get_intersect_days(timestep, domain_config, da_obs_pixel.time, da_mdl_pixel.time, da_pred_pixel.time)
        
        da_obs_sub = da_obs_pixel.loc[dict(time=intersection_day_obs)]
        da_mdl_sub = da_mdl_pixel.loc[dict(time=intersection_day_mdl)]
        da_mdl_sub = da_mdl_sub.stack(ens_time=("ens", "time"), create_index=True)
        da_mdl_sub = da_mdl_sub.drop("time")
        
        da_pred_sub = da_pred_pixel.isel(time=timestep)
        ...
        out[timestep, :] = pred_corr 
    
    #da_out[timestep, :, lat_indx, lon_indx] = pred_corr
